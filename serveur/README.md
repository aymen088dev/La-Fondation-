...
| `RP/` | Resource Pack NaLandia UI (thème or/argent, menus à tuiles JSON UI, HUD) |
...
- `world_behavior_packs.json` → BP `2915bab9-50f2-442e-a17a-4c8e9cd688fb` v2.7.0
- `world_resource_packs.json` → RP `33ca6e1c-4f30-46ae-8b56-1510382e3f61` v2.7.0
...
1. Incrémente `version` dans `BP/manifest.json` ET `RP/manifest.json` (ex : 2.6.0 → 2.7.0)
...
- Le RP contient les textures des formulaires (bandeaux, cartes, panneaux) ET celles des menus à tuiles : cartes de classe (`om_class_*`), barres d'action du clan (`om_clan_tile*`), emplacement vide (`om_slot_empty`) et bannières de drapeau (`om_flag_*`). Les tuiles sont des boutons natifs dessinés par ces images : elles n'interceptent pas les clics et gardent le tactile, la manette et le clavier. Les textures restent remplaçables dans `RP/textures/` en conservant les mêmes noms ; elles sont régénérées par `bun run textures`.
