...
function openPagedList(player, section, bodyText, ordered) {
  let page = 0;
  const showPage = () => {
...
function showActionMenu(player, section, bodyText, ordered) {
  const title = section.replace(/§./g, "").trim();
  const present = (attempt) => {
    system7.runTimeout(() => {
      void (async () => {
        try {
          ...
          for (const action2 of ordered) form.button(action2.label);
          const response = await form.show(player);
          if (response.cancelationReason === "UserBusy" && attempt < 4) {
            present(attempt + 1);
            return;
          }
          if (response.cancelationReason === "UserClosed" || response.selection === void 0) return;
...
// src/ui/tiles.ts
function pageSlice2(items, page, perPage) {
  const pageCount = Math.max(1, Math.ceil(items.length / Math.max(1, perPage)));
  const current = Math.min(Math.max(0, Math.trunc(page)), pageCount - 1);
  return {
    items: items.slice(current * perPage, current * perPage + perPage),
...
async function openDbMenu(db2, player, page = 0) {
  ...
  const { items, page: current, pageCount } = pageSlice2(sections, page, DB_SECTIONS_PER_PAGE);
...
function openMembersMenu(player, manager, territory, page = 0) {
  ...
  const { items, page: current, pageCount } = pageSlice2(data.members, page, MEMBERS_PER_PAGE);
...
function openFlagMenu(player, manager, territory, page = 0) {
  ...
  const { items, page: current, pageCount } = pageSlice2(flags, page, FLAGS_PER_PAGE);
...
function openRolesMenu(player, permissions2, page = 0) {
  ...
  const { items, page: current, pageCount } = pageSlice2(roles, page, ROLES_PER_PAGE);
...
function openRoleMembersMenu(player, role, permissions2, page = 0) {
  ...
  const { items, page: current, pageCount } = pageSlice2(members, page, 5);
  openTileMenu(player, "Roles", (menu) => {
    menu.body(
      [
...
function openPlayersMenu(player, permissions2, db2, page = 0) {
  ...
  const { items, page: current, pageCount } = pageSlice2(entries, page, PLAYERS_PER_PAGE);
...
function openBansList(player, sanctions2, permissions2, page) {
  ...
  const { items, page: current, pageCount } = pageSlice2(bans, page, SANCTIONS_PER_PAGE);
...
function openMutesList(player, sanctions2, permissions2, page) {
  ...
  const { items, page: current, pageCount } = pageSlice2(mutes, page, SANCTIONS_PER_PAGE);
...
function openJobsMenu(player, jobs2, page = 0) {
  ...
  const { items, page: current, pageCount } = pageSlice2(catalog, page, JOBS_PER_PAGE);
...
function openModulesMenu(player, modules2, territories2, page = 0) {
  const { items, page: current, pageCount } = pageSlice2(MODULE_CATALOG, page, MODULES_PER_PAGE);
...
function openQuestMenu(player, quests2, classes2, jobs2, page = 0) {
  ...
  const { items, page: current, pageCount } = pageSlice2(entries, page, QUESTS_PER_PAGE);
